...
function updateProgress() {
  ...
  const floatingCta = qs('#floatingEnrollCta');
  const applicationForm = qs('#application-form');
  if (floatingCta && applicationForm && typeof applicationForm.getBoundingClientRect === 'function') {
    const passedFirstScreen = window.scrollY > Math.max(420, window.innerHeight * .65);
    const formIsNear = applicationForm.getBoundingClientRect().top < window.innerHeight * .9;
    floatingCta.classList.toggle('is-visible', passedFirstScreen && !formIsNear);
  }
}
